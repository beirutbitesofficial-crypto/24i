...
export function TaskStatus({ taskId, current, ar = false }: { taskId: string; current: string; ar?: boolean }) {
  ...
  return <form className="inline-action" onSubmit={submit}><select name="status" defaultValue={current}><option value="TODO">{ar ? "جديدة" : "To do"}</option><option value="IN_PROGRESS">{ar ? "قيد التنفيذ" : "In progress"}</option><option value="REVIEW">{ar ? "مراجعة" : "Review"}</option><option value="REVISION">{ar ? "تعديل" : "Revision"}</option><option value="WAITING_CLIENT">{ar ? "بانتظار العميل" : "Waiting on client"}</option><option value="COMPLETED">{ar ? "مكتملة" : "Completed"}</option></select><input name="note" placeholder={ar ? "ملاحظة (اختياري)" : "Add a note (optional)"} /><button disabled={busy}>{ar ? "حفظ" : "Save"}</button>{message && <small>{message}</small>}</form>;
}
