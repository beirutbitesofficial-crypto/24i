...
export function ContentWorkflow({
  ...
  async function uploadAsset(
    ...
    if (!signRes.ok) {
      throw new Error(signed.error || (ar ? "تعذّر تجهيز الرفع." : "Could not prepare upload."));
    }

    await putFileWithProgress(signed.url, file, mimeType, (loaded, total) => {
    ...
    if (!completeRes.ok) {
      throw new Error(saved.error || (ar ? "تعذّر تسجيل الملف المرفوع." : "Could not register uploaded file."));
    }
  ...
  function caption(e: FormEvent<HTMLFormElement>) {
    ...
    void run(
      ...
      ar
        ? "تم إرسال الفيديو/التصميم مع الكابشن للعميل للموافقة."
  ...
  function schedule(e: FormEvent<HTMLFormElement>) {
    ...
    void run(() =>
      request(`/api/content/${contentId}/schedule`, {
  ...
  function uploadVersion(e: FormEvent<HTMLFormElement>) {
    ...
    if (!files.length) {
      setMessage(
        isCarousel
          ? ar
            ? "اختار صور الكاروسيل أولاً."
            : "Choose the carousel images first."
          : ar
            ? "اختار فيديو أو صورة أولاً."
            : "Choose a video or image first."
      );
      return;
    }
      ...
      async () => {
        ...
        if (isCarousel) {
          ...
          if (files.length !== 1) {
            throw new Error(
              ar
                ? "اختار ملف واحد لهذا النوع من المحتوى."
                : "Choose one file for this content type."
            );
          }
          const file = files[0];
  ...
  function clientDecision(decision: "APPROVED" | "REVISION_REQUESTED") {
    if (decision === "REVISION_REQUESTED" && !clientNote.trim()) {
      setMessage(
        ar
          ? "اكتب ملاحظتك قبل طلب التعديل."
          : "Please write what needs to change before requesting a revision."
      );
      return;
    }
    void run(
      ...
      decision === "APPROVED"
        ? ar
          ? "تمت الموافقة على المحتوى والكابشن. وصل إشعار للمونتير ومدير السوشيال ميديا."
          : "Visual and caption approved. The Editor and Social Media Manager were notified."
        : ar
          ? "تم طلب التعديل وإرسال ملاحظتك للفريق."
    ...
    <div className="management-stack">
        ...
        <section className="panel">
          ...
          {!storageReady && (
            <div className="notice">
              {ar
                ? "التخزين مش مجهّز بعد، لذلك الرفع المباشر متوقف مؤقتاً."
                : "Storage is not configured yet, so direct upload is temporarily disabled."}
            </div>
          )}
          <form className="compact-form upload-version-form" onSubmit={uploadVersion}>
            <label>
              ...
              <input
                name="file"
                type="file"
                accept={isCarousel ? "image/*" : "video/*,image/*"}
                multiple={isCarousel}
                required
              />
            </label>
            <label>
              ...
              <textarea
                name="notes"
                rows={3}
                placeholder={ar ? "ملاحظة اختيارية للفريق" : "Optional production note"}
              />
            </label>
              ...
              {uploadProgress !== null && busy
                ...
                : busy
                  ? ar
                    ...
                    : "Uploading…"
                  : ar
                    ? `رفع V${currentVersion + 1} وإرسالها للـ SMM`
                    : `Upload V${currentVersion + 1} & send to SMM`}
            </button>
          </form>
        </section>
      )}
      ...
      {canWrite && !isClient && (
        <section className="panel">
          ...
          <h2>
            {ar
              ? "أضف الكابشن وأرسل المحتوى كاملاً للعميل"
              : "Add caption & send full content to client"}
          </h2>
          {currentVersion < 1 && (
            <div className="notice">
              {ar
                ? "بانتظار المونتير يرفع الفيديو/التصميم أولاً."
                : "Waiting for the Editor to upload the visual first."}
            </div>
          )}
          <form className="compact-form" onSubmit={caption}>
            <label>
              {ar ? "الكابشن" : "Caption"}
              <textarea name="caption" rows={6} defaultValue={captionText || ""} required />
            </label>
            <label>
              {ar ? "الهاشتاغات" : "Hashtags"}
              <textarea name="hashtags" rows={2} defaultValue={captionHashtags || ""} />
            </label>
            <label>
              CTA
              <input name="cta" defaultValue={captionCta || ""} />
            </label>
            <button disabled={busy || currentVersion < 1}>
              {busy
                ? ar
                  ...
                  : "Sending…"
                : ar
                  ? "إرسال الفيديو/التصميم + الكابشن للعميل"
                  : "Send visual + caption to client"}
            </button>
          </form>
        </section>
      )}
            ...
            <>
              ...
              <div className="client-caption-preview">
                <span className="muted">
                  {ar ? `الكابشن V${currentCaptionVersion}` : `Caption V${currentCaptionVersion}`}
                </span>
                <p>{captionText}</p>
              ...
              <label>
                {ar ? "ملاحظات التعديل" : "Revision notes"}
                <textarea
                  value={clientNote}
                  onChange={(e) => setClientNote(e.target.value)}
                  rows={4}
                  placeholder={ar ? "مطلوبة فقط إذا بدك تعديل" : "Only required if you want changes"}
                />
              </label>
              <div className="review-actions">
                <button
                  type="button"
                  disabled={busy}
                  onClick={() => clientDecision("APPROVED")}
                >
                  {ar ? "موافق على المحتوى + الكابشن" : "Approve visual + caption"}
                </button>
                <button
                  type="button"
                  className="revision-button"
                  disabled={busy}
                  onClick={() => clientDecision("REVISION_REQUESTED")}
                >
                  {ar ? "طلب تعديل" : "Request revision"}
                </button>
              </div>
            </>
          ) : (
