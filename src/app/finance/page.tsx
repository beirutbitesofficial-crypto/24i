...
export default async function Finance() {
  ...
  if (user.role.key === "CLIENT") {
    ...
    return <AppShell user={user} title="Payments" kicker="MY ACCOUNT"><div className="metrics"><article><span>Current package</span><b>{currentPackage ? `$${currentPackage.price.toFixed(2)}` : "—"}</b></article><article><span>Paid</span><b>${paid.toFixed(2)}</b></article><article><span>Remaining</span><b>${remaining.toFixed(2)}</b></article></div>{currentPackage&&<section className="panel"><span className="eyebrow">PACKAGE</span><h2>{currentPackage.package.name}</h2><p>{currentPackage.client.brandName} · {currentPackage.package.interval}</p><p className="muted">Started {currentPackage.startsAt.toLocaleDateString()}{currentPackage.endsAt?` · Ends ${currentPackage.endsAt.toLocaleDateString()}`:""}</p></section>}<div className="panel tablewrap"><table><thead><tr><th>Invoice</th><th>Client</th><th>Total</th><th>Paid</th><th>Remaining</th><th>Status</th><th>Due</th></tr></thead><tbody>{invoices.map((x) => { const invoicePaid=x.payments.reduce((s,p)=>s.plus(p.amount),money(0)); const due=x.total.minus(invoicePaid); return <tr key={x.id}><td><b>{x.number}</b></td><td>{x.client.brandName}</td><td>${x.total.toFixed(2)}</td><td>${invoicePaid.toFixed(2)}</td><td>${due.toFixed(2)}</td><td>{x.status.replaceAll("_"," ")}</td><td>{x.dueDate.toLocaleDateString()}</td></tr>; })}</tbody></table>{!invoices.length&&<p>No invoices yet.</p>}</div></AppShell>;
  }

  if (!hasPermission(user, "finance.read")) redirect("/");
  const canPackages=hasPermission(user,"packages.write"),canInvoice=hasPermission(user,"finance.invoices.write"),canPayment=hasPermission(user,"finance.payments.write"),canExpense=hasPermission(user,"finance.expenses.write"),canSalary=hasPermission(user,"finance.salaries.write");
