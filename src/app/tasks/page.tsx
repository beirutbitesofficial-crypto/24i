...
export default async function Tasks() {
  ...
  const [rows, clients, team] = await Promise.all([
    ...
    hasPermission(user, "tasks.write") ? db.client.findMany({ where: ids ? { id: { in: ids } } : {}, select: { id: true, brandName: true }, orderBy: { brandName: "asc" } }) : Promise.resolve([]),
    hasPermission(user, "tasks.write") ? db.user.findMany({ where: { status: "ACTIVE", role: { key: { in: ["ADMIN","MANAGER","EDITOR","SOCIAL_MEDIA_MANAGER"] } } }, include: { role: true }, orderBy: { name: "asc" } }) : Promise.resolve([]),
  ]);

  return <AppShell user={user} title="Tasks" kicker="WORKLOAD">
    <div className="management-stack">
      {hasPermission(user, "tasks.write") && <TaskManager clients={clients} users={team.map((u) => ({ id: u.id, name: u.name, role: u.role.name }))} />}
